...
function buildAggregations(orgs) {
  ...
  const timeBuckets = [
    { label: '0â14 days', min: 0, max: 14 },
    { label: '15â30 days', min: 15, max: 30 },
    { label: '31â60 days', min: 31, max: 60 },
    { label: '61â100 days', min: 61, max: 100 },
  ...
  // Use Stripe billed users if available, fall back to latest_snapshot_billable_users
  const postActual = postLaunch.map(o => o.stripe_billed_users ?? o.latest_snapshot_billable_users);
  const postPotential = postLaunch.map(o => Math.max(o.stripe_billed_users ?? o.latest_snapshot_billable_users, o.active_user_count + o.needs_invite_count));
...
function buildDashboardData(csBilling, bonusesPaid, missingInvites, stripeData = {}) {
  ...
  const orgs = merged.map(deriveOrgFields).filter(o => !o.is_canceled).map(o => {
    // Enrich with Stripe billed users and dashboard link
    const subId = o.stripe_subscription_id;
    o.stripe_billed_users = (subId && stripeData[subId] != null) ? stripeData[subId] : null;
    o.stripe_customer_url = o.stripe_customer_id
